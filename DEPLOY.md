...
- 배포 끝에 `docker system prune -af` 로 옛 이미지·정지 컨테이너·빌드 캐시를 정리해 1코어 VPS 디스크를 확보한다. `--volumes` 를 주지 않아 `postgres-data` 볼륨은 보존되고, 실행 중인 app·cloudflared 이미지는 사용 중이라 삭제되지 않는다.
