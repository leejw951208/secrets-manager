...
- **접근 제어.** 데이터 라우트(`/sites`·`/categories`·`/secrets`·`/search`·`/store`)는 passkey 인증 가드 + CSRF(Origin 검증 + `X-Vault-Request` 헤더)로 보호한다. 비밀번호 본문 작업(생성·열람·수정)은 잠금해제(PRF 세션 키)를 추가로 요구한다.
